...
namespace TWD_Systems
{
    ...
    public class NavMeshQuerySystem : JobComponentSystem
    {
        ...
        public List<float3> a1;
        public List<float3> b1;
            ...
            while (QueryQueue.Count > 0 && availableSlots.Count > 0)
            {
                if (QueryQueue.TryDequeue (out PathQueryData pending))
                {
                    ...
                    else if (availableSlots.TryDequeue (out int index))
                    {
                        ...
                        {
                            query.Dispose();
                            pathFailedCallbacks?.Invoke(pending.id, PathfindingFailedReason.InvalidFromLocation);
                        ...
                        {
                            query.Dispose();
                            pathFailedCallbacks?.Invoke(pending.id, PathfindingFailedReason.InvalidToLocation);
                            availableSlots.Enqueue(index);
                            continue;
                        }
